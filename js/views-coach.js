(function(){
  ...
  function bind(route){
    ...
    document.querySelectorAll('.session-swap').forEach(sel=>sel.addEventListener('change',()=>{window.V14State.setSelection(sel.dataset.session,sel.dataset.slotKey,sel.value);const current=window.V14Router.parseHash(location.hash);document.getElementById('app-main').innerHTML=render(current);bind(current);}));
    const status=document.getElementById('copy-session-status');
