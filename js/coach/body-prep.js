(function(){
  ...
  function foamRollCardsFor(session){
    if(!session||!M.Foam?.foamRollCardsFor)return '';
    const family=window.V14_DATA?.bodyFamilies?.[session.familyId]?.name||session.familyId||'Body';
    return M.Foam.foamRollCardsFor({session,level:session.level||'L1',subtitle:`${family} · ${session.level||'L1'}`});
  }
