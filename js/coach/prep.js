(function(){
  ...
  function legacyMatchedWarmups(recipeId,level){
    const data=D(),recipe=data.recipes[recipeId]||{},tier=`T${level.slice(-1)}`,gradeApi=G();
    const eligible=id=>{
  ...
  function warmupCards(recipeId,level,mainActionIds){
    const data=D(),recipe=data.recipes[recipeId]||{},tier=`T${level.slice(-1)}`,items=matchedWarmups(recipeId,level,mainActionIds),grades=G()?.allowedGrades(level)||[];
  ...
  function composerPrepHtml(ctx){
    const items=composerPrepItems(ctx),foam=M.Foam.composerFoamItems(ctx),title=`${ctx.resolved.lower.name} + ${ctx.resolved.upper.name} · ${ctx.level}`,grades=G()?.allowedGrades(ctx.level)||[];
    ...
    return `<section class="section-card"><div class="section-head"><div><h2>2F｜PREP</h2><p>根据当前实际 A / B anatomy 自动匹配；PREP 等级按 ${esc(ctx.level)} 的 ${esc(grades.join(' → ')||'—')} 向下兼容窗口排序，主动作 T 只提供训练上下文。</p></div><span class="time-badge">约 10–12 分钟</span></div><div class="session-prep-match session-foam-match"><div class="prep-match-head"><div><b>泡沫轴推荐</b><span>${esc(title)}</span></div><div class="prep-match-head-actions">${foamCopy}</div></div><div class="session-foam-grid">${foamCards}</div></div><div class="session-prep-match"><div class="prep-match-head"><div><b>动态热身 / 激活</b><span>${esc(title)} · PREP ${esc(grades.join(' → ')||'—')}</span></div><div class="prep-match-head-actions">${warmCopy}</div></div><div class="session-warmup-grid">${warmCards}</div></div></section>`;
  }
